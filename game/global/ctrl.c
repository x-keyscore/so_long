...
/*   Updated: 2024/02/07 03:37:30 by anraymon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ctrl_handler(t_vars *vars, int state)
{
	if (state == 0 && vars->ctrl_state == 1)
	{
		if (vars->spaceship[0].dir == 0)
			vars->player[0].axis.x = vars->spaceship[0].axis.x + (vars->spaceship[0].size.w);
		else if (vars->spaceship[0].dir == 1)
			vars->player[0].axis.x = vars->spaceship[0].axis.x - (vars->player[0].size.w);
		vars->player[0].axis.y = vars->spaceship[0].axis.y;
		if (!ground_collision(vars, vars->player[0].axis, vars->player[0].size.w, vars->player[0].size.h))
		{
